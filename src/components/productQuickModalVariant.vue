      ...
      >
        ...
        >
          <button
            ...
            @click="selectOption(attribute.productoption, option); "
          >
            <img
        ...
        >
          ...
          <template v-else>
            <button
              ...
              :key="option.id"
              type="button"
              class="focus:ring-2 ring-[var(--btn-primary-solid-background)] ring-offset-2 flex items-center justify-center rounded-md border px-3 py-3 text-sm font-medium uppercase transition-all duration-200"
              :class="{
                'border-transparent bg-[var(--btn-primary-solid-background)] text-white hover:bg-[var(--btn-primary-solid-background-hover)]': selectedOptions[attribute.productoption]?.id === option.id,
...
import { useSelectedQuickModalVariants } from '@/composables/useSelectedQuickModalVariants.js';
...
const { setSelectedVariant } = useSelectedQuickModalVariants();
// Reactive state
