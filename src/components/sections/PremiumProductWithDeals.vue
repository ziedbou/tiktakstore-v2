<template>
  <div class="deal premium-product-with-deals">
    <div class="row justify-content-center">
      <div class="col-sm-12 col-md-8 col-lg-6">
        <div class="deal-content text-center">
          ...
          <div class="premium-product-with-deals-timer my-5 d-flex justify-content-center">
            <div class="time-block">
              <div class="time-row">
                <span class="time-block__num">{{ String(countdown.days).padStart(2, '0') }}</span>
            ...
            <div class="time-block">
              <div class="time-row">
                <span class="time-block__num">{{ String(countdown.hours).padStart(2, '0') }}</span>
                <span class="separator">:</span>
              </div>
            ...
            <div class="time-block">
              <div class="time-row">
                <span class="time-block__num">{{ String(countdown.minutes).padStart(2, '0') }}</span>
                <span class="separator">:</span>
              </div>
            ...
            <div class="time-block">
              <div class="time-row">
                <span class="time-block__num">{{ String(countdown.seconds).padStart(2, '0') }}</span>
              </div>
          ...
          <div 
            ...
            class="premium-product-with-deals-timer-completed-text"
          >
          </div>
        </div>
        <div v-if="!product " class="row my-5 mx-5 justify-content-center">
          <div class="col-7">
        ...
        <div v-if="product" class="row deal-products justify-content-center">
          <div 
            class="col-7 deal-product text-center mb-10"
          >
...
const { data: product, pending, error, refresh } = useGetProducts({
  type: 'single',
  cacheKey: `PremiumProductWithDeals-${props.index}`,
  companyId: companyData.value.id,
  productList:  {products_ids: [props.data.values?.product_1 ? props.data.values.product_1: "0"] }
});
...
.premium-product-with-deals .deal-content h4 {
...
.premium-product-with-deals .deal-content h2 {
  font-weight: 500;
...
.premium-product-with-deals .premium-product-with-deals-timer {
  display: flex;
  align-items: center;
  gap: 16px; 
}

.premium-product-with-deals .premium-product-with-deals-timer .time-row {
  display: flex;
  align-items: center;
...
.premium-product-with-deals .premium-product-with-deals-timer .time-block__num {
  min-width: 27px;
  text-align: center;
}

.premium-product-with-deals .premium-product-with-deals-timer .separator {
  position: absolute;
  top: 50%;
...
.premium-product-with-deals .premium-product-with-deals-timer .time-block__unit {
  display: block;
  width: 100%;
  text-align: center;
  margin: 0px;
...
.premium-product-with-deals .product-media {
  border-radius: v-bind('data.values.image_border_radius + "px"');
}

.premium-product-with-deals .deal-product .action {
  font-size: 14px;
  font-weight: 400;
  border-radius: 20px;
...
.premium-product-with-deals {
  ...
  .col-sm-12, .col-md-8, .col-lg-6, .col-7 {
    padding-left: 15px;
    padding-right: 15px;
  }
