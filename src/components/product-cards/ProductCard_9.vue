    ...
    <div class="product_cart h-full group relative overflow-hidden hover:shadow-md transition-all duration-300">
      <NuxtLink :to="getProductLink(product)" :class="[image_full ? 'p-0' : 'p-2 pb-0', 'relative flex']">
        <img
          ...
          :alt="product.name"
          :style="{ borderRadius: image_full ? '0' : 'var(--product-card-border-radius, 5px)' }"
          class="h-96 w-full object-cover group-hover:opacity-75 sm:aspect-2/3 sm:h-auto"
        />
        <Tags
          :product="product"
      ...
      <div class="p-2">
        ...
        <span v-if="product.discount" class="line-through text-sm">{{ product.price }} {{ /*company.currency*/ "TND" }}</span>
        </p>
      </div>
    </div>
      <!-- Quick View Modal -->
